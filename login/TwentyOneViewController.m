...
- (void)viewDidLoad {
   ...
   // [_next setImage:[UIImage imageNamed:@"继续答题.png"] forState:UIControlStateNormal];
    ...
    if ([user objectForKey:@"english"]) {
        ...
        _lb1.text=@"47、If Catheter, the type of the arterial catheter was (as shown in the pictures)";
        _lb2.text=@"48、If Catheter, the length of catheter indwelling was";
        ...
        _twoArray=[[NSMutableArray alloc] initWithObjects:@"≤4days",@">4days",nil];
        _oneArray=[[NSMutableArray alloc] initWithObjects:@"Open",@"Closed",nil];
...
#pragma 第一个问题
    ...
    for (int i=0; i<_oneArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( i*(Button_Width-100 + Width_Space+100) + Start_X,  (Button_Height + Height_Space)+Start_Y-130, Button_Width-100, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
        NSString *a1 = [_oneArray objectAtIndex:i];
        
        [cb setText:a1];
        
        
        [cb setStateChangedTarget:self selector:@selector(change47:)];
...
#pragma 第二个问题
    ...
    for (int i=0; i<_twoArray.count; i++) {
        ...
        [cb setStateChangedTarget:self selector:@selector(change48:)];
...
-(void)change47:(SSCheckBoxView*)cbv{
...
-(void)change48:(SSCheckBoxView*)cbv{
...
- (IBAction)nextBtn:(id)sender {
    ...
    for(int i=0;i<[_chooseArray1 count];i++)
    {
        ...
        if (pp.checked==1&&[pp.textLabel.text isEqualToString:[_oneArray lastObject]]) {
            ...
            [fengbi setObject:@"1" forKey:@"fengbi"];
            [fengbi synchronize];
        }
        else if(pp.checked==1&&[pp.textLabel.text isEqualToString:[_oneArray firstObject]]){
            NSUserDefaults *fengbi = [NSUserDefaults standardUserDefaults];
            [fengbi removeObjectForKey:@"fengbi"];
            [fengbi synchronize];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex==0) {
        TwentyOneViewController *receive = [self.storyboard instantiateViewControllerWithIdentifier:@"FourPage"];
