...
@interface eighteen1ViewController ()
...
@implementation eighteen1ViewController
...
- (void)viewDidLoad {
   ...
   // [_next setImage:[UIImage imageNamed:@"继续答题.png"] forState:UIControlStateNormal];
    ...
    if ([user objectForKey:@"english"]) {
        ...
        _twoArray=[[NSMutableArray alloc] initWithObjects:@"动脉穿刺采血",@"动脉导管采血",@"静脉穿刺采血",@"静脉导管采血",nil];
        _threeArray=[[NSMutableArray alloc] initWithObjects:@"<3分钟",@"3-5分钟",@"5-10分钟",@"≥10分钟",nil];
        _oneArray=[[NSMutableArray alloc] initWithObjects:@"2次",@"3次",@"4次",@"4次以上",nil];
...
#pragma 第一个问题
    ...
    for (int i=0; i<_oneArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( i*(Button_Width-50 + Width_Space) + Start_X,  (Button_Height + Height_Space)+Start_Y-250, Button_Width, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
...
#pragma 第二个问题
    ...
    for (int i=0; i<_twoArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( index*(Button_Width-100 + Width_Space+200) + Start_X,  page*(Button_Height + Height_Space)+Start_Y-50, Button_Width+100, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
...
#pragma 第三个问题
    ...
    for (int i=0; i<_threeArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( i*(Button_Width-60 + Width_Space) + Start_X,  (Button_Height + Height_Space)+Start_Y+80, Button_Width-60, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex==0) {
        eighteen1ViewController *receive = [self.storyboard instantiateViewControllerWithIdentifier:@"FourPage"];
...
- (IBAction)nextBtn:(id)sender {
    ...
    if (!isok1||!isok2) {
       ...
       // NSDictionary *q3 =@{@"choose":Q3};
        ...
        NSDictionary*num1=@{@"43.1":q1};
        NSDictionary*num2=@{@"43.2":q2};
       // NSDictionary*num3=@{@"43":q3};
