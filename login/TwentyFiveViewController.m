...
@property(nonatomic,strong)TwentySixViewController*twentysix;
...
- (void)viewDidLoad {
    ...
    if ([user objectForKey:@"english"]) {
        ...
        _lb1.text=@"60、Was the sharps container placed within the arm reach of the operator?";
        _lb2.text=@"61、Were examination and emptying of air bubbles performed before the samples were closed?";
        _lb3.text=@"62、Were the mixing steps carried out correctly after the samples were closed (if \"arterial blood collector\", gently reverse the collector upside down five times and then rub it with hands for 5 seconds)? ";
       ...
       [_photoBtn setImage:[UIImage imageNamed:@"建议拍照英文.png"] forState:UIControlStateNormal];
        ...
        _threeArray=[[NSMutableArray alloc] initWithObjects:@"YES",@"No, mixing was performed but did not reach the standards",@"No, mixing was not performed",nil];
...
#pragma 第一个问题
    ...
    for (int i=0; i<_oneArray.count; i++) {
        ...
        [cb setStateChangedTarget:self selector:@selector(change60:)];
...
#pragma 第二个问题
    ...
    for (int i=0; i<_twoArray.count; i++) {
        ...
        [cb setStateChangedTarget:self selector:@selector(change61:)];
...
#pragma 第三个问题
    ...
    for (int i=0; i<_threeArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( index*(Button_Width+30 + Width_Space) + Start_X,  page*(Button_Height + Height_Space)+Start_Y+80, Button_Width+400, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
        NSString *a1 = [_threeArray objectAtIndex:i];
        
        [cb setText:a1];
        
        
        [cb setStateChangedTarget:self selector:@selector(change62:)];
...
-(void)change60:(SSCheckBoxView*)cbv{
...
-(void)change61:(SSCheckBoxView*)cbv{
...
-(void)change62:(SSCheckBoxView*)cbv{
...
- (IBAction)nextBtn:(id)sender {
    ...
    if (!isok1||!isok2||!isok3) {
        ...
        NSDictionary*num1=@{@"60":q1};
        NSDictionary*num2=@{@"61":q2};
        NSDictionary*num3=@{@"62":q3};
...
//        TwentyFiveViewController *receive = [self.storyboard instantiateViewControllerWithIdentifier:@"26Page"];
//        
//        [self.navigationController pushViewController:receive animated:YES];

        if (!_twentysix) {
            
            _twentysix= [self.storyboard instantiateViewControllerWithIdentifier:@"26Page"];
        }
        
        
        
        [self.navigationController pushViewController:_twentysix animated:YES];
    }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex==0) {
        TwentyFiveViewController *receive = [self.storyboard instantiateViewControllerWithIdentifier:@"FourPage"];
