...
@interface TwentyTwo1ViewController ()
...
@implementation TwentyTwo1ViewController
   ...
   // [_next setImage:[UIImage imageNamed:@"继续答题.png"] forState:UIControlStateNormal];
    ...
    if ([user objectForKey:@"english"]) {
        ...
        _lb3.text=@"43、If \"Puncture\", the length of pressing was";
         ...
         _oneArray=[[NSMutableArray alloc] initWithObjects:@"注射器-手动抽拉",@"动脉采血器-手动抽拉",@"动脉采血器-待血液自然上涌至设定刻度",nil];

        _threeArray=[[NSMutableArray alloc] initWithObjects:@"开放式三通",@"肝素帽",@"机械接头",@"分隔膜接头",@"软针采血窗（导管自带）",nil];
...
#pragma 第一个问题
    ...
    for (int i=0; i<_oneArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( i*(Button_Width-40 + Width_Space) + Start_X,  (Button_Height + Height_Space)+Start_Y-250, Button_Width+150, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
...
#pragma 第三个问题
    ...
    for (int i=0; i<_threeArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( i*(Button_Width-120 + Width_Space) + Start_X,  (Button_Height + Height_Space)+Start_Y+150, Button_Width+20, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex==0) {
        TwentyTwo1ViewController *receive = [self.storyboard instantiateViewControllerWithIdentifier:@"FourPage"];
...
- (IBAction)nextBtn:(id)sender {
    ...
    if (!isok1||!isok2||!isok3) {
        ...
        NSDictionary*num1=@{@"48.1":q1};
        NSDictionary*num2=@{@"48.2":q2};
        NSDictionary*num3=@{@"48.3":q3};
...
//        [self.navigationController pushViewController:receive animated:YES];
        ...
        TwentyTwo1ViewController *receive = [self.storyboard instantiateViewControllerWithIdentifier:@"22Page"];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if(imagePoint.x <= touchPoint.x && imagePoint.x +_img1.frame.size.width >=touchPoint.x && imagePoint.y <=  touchPoint.y && imagePoint.y+_img1.frame.size.height >= touchPoint.y)
    {
        ...
        if (_isFullScreen) {
            ...
            // 缩小尺寸
            _img1.frame = CGRectMake(816, 27, 50, 50);
        }
