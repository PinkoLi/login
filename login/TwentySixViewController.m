    ...
    if (!isok1) {
        ...
        if ([[user objectForKey:@"diaoyan"]count]==0) {
          ...
          NSMutableArray* allArr=[NSMutableArray arrayWithObjects:arr,nil];
            ...
            [user synchronize];
             [choose synchronize];
            NSLog(@"111111111111111%@",[user objectForKey:@"diaoyan"]);
...
-(NSString *)getImageSavePath{
    ...
    NSString*str=[[@"63_" stringByAppendingString:photoName] stringByAppendingString:@".jpg"];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if(imagePoint.x <= touchPoint.x && imagePoint.x +_img1.frame.size.width >=touchPoint.x && imagePoint.y <=  touchPoint.y && imagePoint.y+_img1.frame.size.height >= touchPoint.y)
    {
        ...
        if (_isFullScreen) {
            ...
            // 缩小尺寸
            _img1.frame = CGRectMake(800, 27, 50, 50);
        }
