...
- (void)viewDidLoad {
    ...
    if ([user objectForKey:@"english"]) {
        ...
        _lb1.text=@"53、If \"Syringe\", was the syringe needle closed with rubber stopper immediately after blood collection?";
        _lb2.text=@"54、If \"safety-type arterial blood collector\", was the safety lock activated immediately after blood collection?";
        _lb3.text=@"55、If \"safety-type arterial blood collector\", was the safety lock activated correctly after blood collection (press the safety button with one hand)?";
...
#pragma 第一个问题
    ...
    for (int i=0; i<_oneArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( i*(Button_Width-100 + Width_Space) + Start_X,  (Button_Height + Height_Space)+Start_Y-250, Button_Width, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
        NSString *a1 = [_oneArray objectAtIndex:i];
        
        [cb setText:a1];
        
        
        [cb setStateChangedTarget:self selector:@selector(change53:)];
        
        [_chooseArray1 addObject:cb];
        [_view2 addSubview:cb];
        
        if ([[zhushe objectForKey:@"zhushe"]isEqualToString:@"1"]) {
            cb.hidden=YES;
            _lb1.hidden=YES;
            _btn1.hidden=YES;
            _img1.hidden=YES;
...
#pragma 第二个问题
    ...
    for (int i=0; i<_twoArray.count; i++) {
        ...
        [cb setStateChangedTarget:self selector:@selector(change54:)];
...
#pragma 第三个问题
    ...
    for (int i=0; i<_threeArray.count; i++) {
        ...
        SSCheckBoxView*cb = [[SSCheckBoxView alloc] initWithFrame:CGRectMake( i*(Button_Width-100 + Width_Space) + Start_X,  (Button_Height + Height_Space)+Start_Y+130, Button_Width-100, Button_Height) style:kSSCheckBoxViewStyleMono checked:NO];
        NSString *a1 = [_threeArray objectAtIndex:i];
        
        [cb setText:a1];
        
        
        [cb setStateChangedTarget:self selector:@selector(change55:)];
...
-(void)change53:(SSCheckBoxView*)cbv{
...
-(void)change54:(SSCheckBoxView*)cbv{
...
-(void)change55:(SSCheckBoxView*)cbv{
...
- (IBAction)nextBtn:(id)sender {
    ...
    if ([[zhushe objectForKey:@"zhushe"] isEqualToString:@"0"]) {
...
//        TwentyThreeViewController *receive = [self.storyboard instantiateViewControllerWithIdentifier:@"24Page"];
//        
//        [self.navigationController pushViewController:receive animated:YES];
        ...
        if (!isok2||!isok3) {
            ...
            NSDictionary*num2=@{@"54":q2};
            NSDictionary*num3=@{@"55":q3};
...
//        NSString*photoName=[NSString stringWithFormat:@"%d",[[photoList objectForKey:@"photoList"] integerValue]];
        ...
        _str=[[@"53_" stringByAppendingString:date] stringByAppendingString:@".jpg"];
    }
    else if(_a==1){
...
//        NSString*photoName=[NSString stringWithFormat:@"%d",[[photoList objectForKey:@"photoList"] integerValue]];
        ...
        formatter.dateFormat = @"yyyyMMddHHmmss";
        NSString *date = [formatter stringFromDate:[NSDate date]];
        _str=[[@"54_" stringByAppendingString:date] stringByAppendingString:@".jpg"];
    }
    else{
//        NSUserDefaults *photoList = [NSUserDefaults standardUserDefaults];
//        NSString*photoName=[NSString stringWithFormat:@"%d",[[photoList objectForKey:@"photoList"] integerValue]];
        ...
        _str=[[@"55_" stringByAppendingString:date] stringByAppendingString:@".jpg"];
    }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex==0) {
        TwentyThreeViewController *receive = [self.storyboard instantiateViewControllerWithIdentifier:@"FourPage"];
